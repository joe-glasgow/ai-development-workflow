...
class AIIntegration {
  ...
  private buildContextualPrompt(userPrompt: string, personaContext: string, context: WorkflowContext): string {
    let prompt = '';
  ...
  private async callOpenAI(prompt: string, config: AIProvider): Promise<string> {
  ...
  private async callAnthropic(prompt: string, config: AIProvider): Promise<string> {
    const response = await axios.post(
  ...
  private async callOllama(prompt: string, config: AIProvider): Promise<string> {
    const response = await axios.post(
      `${config.baseUrl}/chat/completions`,
      {
        model: config.model,
...
Follow best practices for ${answers.codeType} development.`;
  ...
  private getPersonaForCodeType(codeType: string): string {
...
program
  .name('ai-workflow')
  .description('AI integration for development workflow')
  .version('1.0.0');
...
program
  .command('export')
  .description('Export AI session data')
  .action(async () => {
