...
class PersonaManager {
  ...
  private getTemplateName(projectType: string): string {
  ...
  private getPhasePersonas(phaseName: string): string[] {
...
program
  ...
  .description('Initialize a new AI-powered development project')
  .action(async (projectName) => {
    await manager.initProject(projectName);
  });

program
  .command('list-personas')
  .description('List available personas for the current project')
  .action(async () => {
    await manager.listPersonas();
  });

program
  .command('get-persona')
  .argument('<persona-name>', 'Name of the persona to retrieve')
  .description('Get persona context for AI interactions')
  .action(async (personaName) => {
    await manager.getPersona(personaName);
  });

program
  .command('start-phase')
  .argument('<phase-name>', 'Name of the phase to start (discovery, design, development, etc.)')
  .description('Start a new workflow phase')
  .action(async (phaseName) => {
    await manager.startPhase(phaseName);
  });

program
  .command('complete-phase')
  .description('Mark the current phase as completed')
  .action(async () => {
